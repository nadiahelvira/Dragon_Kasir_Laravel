							...
							<!-- batas scroll --> 
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th width="200px">
                                            ...
                                            <label for="BACNO" class="form-label">Acno</label></th>
                                        <th width="200px">Nama</th>
                                        <th width="600px">Uraian</th>
                                        <th width="200px">Debet</th>
								...
								@foreach ($detail as $detail)
                                    <tr>
										...
										<td>
                                            <input name="DEBET[]" onclick="select()" onkeyup="hitung()" id="DEBET{{$no}}" type="text" value="{{$detail->DEBET}}"
                                            class="form-control DEBET" style="text-align: right"  required>
                                        </td>
										<td>
                                            <input name="KREDIT[]" onclick="select()" onkeyup="hitung()" id="KREDIT{{$no}}" type="text" value="{{$detail->KREDIT}}"
                                            class="form-control KREDIT" style="text-align: right"  required>
                                        </td>
								...
								<tfoot>
									...
									<td></td>
                                    <td></td>
                                    <td><input class="form-control TDEBET text-primary font-weight-bold" style="text-align: right"  id="TDEBET" name="TDEBET" value="{{$header->DEBET}}" readonly></td>
                                    <td><input class="form-control TKREDIT text-primary font-weight-bold" style="text-align: right"  id="TKREDIT" name="TKREDIT" value="{{$header->KREDIT}}" readonly></td>
                                    <td></td>
                                </tfoot>
                            </table>
...
/////////////////////////////////////////////
            ...
            function simpan() {
				...
				var TDEBETX = $('#TDEBET').val();
				var TKREDITX = $('#TKREDIT').val();
   ...
   function hitung() {
		...
		if(isNaN(TDEBET)) TDEBET = 0;
		if(isNaN(TKREDIT)) TKREDIT = 0;

		$('#TJUMLAH').val(numberWithCommas(TJUMLAH));
		$("#TJUMLAH").autoNumeric('update');
		
		$('#TDEBET').val(numberWithCommas(TDEBET));
		$("#DEBET").autoNumeric('update');

		$('#TKREDIT').val(numberWithCommas(TKREDIT));
		$("#KREDIT").autoNumeric('update');
	...
	function hidup() {
		...
		for (i = 0; i <= jumlahdata; i++) {
			...
			$("#DEBET" + i.toString()).attr("readonly", false);
			$("#KREDIT" + i.toString()).attr("readonly", false);
	...
	function mati() {
		...
		for (i = 0; i <= jumlahdata; i++) {
			...
			$("#DEBET" + i.toString()).attr("readonly", true);
			$("#KREDIT" + i.toString()).attr("readonly", true);
    ...
    function tambah() {
	  ...
	  html=`<tr>
				...
				<td>
		            <input name='DEBET[]'  onblur='hitung()' value='0' id='DEBET${idrow}' type='text' style='text-align: right' class='form-control DEBET text-primary' required >
                </td>
				<td>
		            <input name='KREDIT[]'  onblur='hitung()' value='0' id='KREDIT${idrow}' type='text' style='text-align: right' class='form-control KREDIT text-primary' required >
                </td>
		...
		for (i = 0; i <= jumlahdata; i++) {
			$("#JUMLAH" + i.toString()).autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
			$("#DEBET" + i.toString()).autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
			$("#KREDIT" + i.toString()).autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
		}
