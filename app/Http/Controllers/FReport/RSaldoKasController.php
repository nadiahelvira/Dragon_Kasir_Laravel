...
class RSaldoKasController extends Controller
{
   ...
   public function report()
    {
		...
		//return $acno;
        return view('freport_saldokas.report')->with(['acno' => $acno])->with(['per' => $per])->with(['hasil' => []]);
		
    }

    	public function jasperSaldoKasReport(Request $request) 
	{
		$file 	= '';
		...
		$query = DB::SELECT("SELECT account.ACNO AS ACNO,
			...
			AND accountd.KEL = 100
			GROUP BY accountd.KEL = 100;
		");
		...
		if($request->has('filter'))
		{
			$acno = Account::query()->get();
			$per = Perid::query()->get();
			return view('freport_saldokas.report')->with(['acno' => $acno])->with(['per' => $per])->with(['hasil' => $query]);
		}
