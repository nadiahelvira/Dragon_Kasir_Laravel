...
class ROperbbmbbkController extends Controller
{
    ...
    public function report()
    {
        ...
        return view('oreport_operbbmbbk.report')->with(['hasil' => []]);
    }
	 
	 
	public function jasperOperbbmbbkReport(Request $request) 
	{
		...
		if ( $request->pilihan =='0' )
		{
			$query = DB::SELECT(
					...
					FROM bank WHERE bank.PER='$per' AND TRIM(TYPE)=TRIM('$TYPEX') AND TRIM(NO_BUKTI) >= TRIM('$NOMAW') AND  TRIM(NO_BUKTI) <= TRIM('$NOMAK')
		...
		else
		{
			  ...
			  DB::SELECT('call akt_opernobuktibank_all(?,?,?,?,?)', array($TYPEX, $per, $NOMAW, $NOMAK, $NOMX));
		      $query = DB::SELECT(
		...
		return view('oreport_operbbmbbk.report')->with(['hasil' => $query]);
