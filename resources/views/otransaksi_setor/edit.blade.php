...
<div class="content-wrapper">
    ...
    <div class="content-header">
        ...
        <div class="row mb-2">
            <div class="col-sm-6">
               <h1 class="m-0">Edit Koreksi Setoran Bahan {{$header->NO_BUKTI}}</h1>	   
            </div>
            <!-- /.col -->
            <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{url('/setor')}}">Setoran Bahan</a></li>
    ...
    <div class="content">
        ...
        <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{url('/setor/update/'.$header->NO_ID)}}" method="POST">
						@csrf
                            ...
                            <table id="datatable" class="table table-striped table-border">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="text-align: center;">BB-BP</th>										
                                        <th></th>	
                                    </tr>
								...
								<?php $no=0 ?>
								@foreach ($detail as $setord)
                                    <tr>
                                        <td>
										    <input type="hidden" name="NO_ID[]" id="NO_ID{{$no}}" type="text" value="{{$setord->NO_ID}}" 
                                            class="form-control NO_ID" onkeypress="return tabE(this,event)" readonly>
                                            
											<input name="REC[]" id="REC{{$no}}" type="text" value="{{$setord->REC}}" class="form-control REC" onkeypress="return tabE(this,event)" >
                                        </td>
                                        <td>
                                            <input name="KD_BRG[]" id="KD_BRG{{$no}}" type="text" value="{{$setord->KD_BRG}}" class="form-control KD_BRG" readonly required>
                                        </td>
                                        <td>
                                            <input name="NA_BRG[]" id="NA_BRG{{$no}}" type="text" value="{{$setord->NA_BRG}}" class="form-control NA_BRG" readonly required>
                                        </td>
                                        <td>
                                            <input name="SATUAN[]" id="SATUAN{{$no}}" type="text" value="{{$setord->SATUAN}}" class="form-control SATUAN" readonly required>
                                        </td>
										
										<td><input name="QTY[]" onclick="select()" onkeyup="hitung()" value="{{$setord->QTY}}" id="QTY{{$no}}" type="text" style="text-align: right"  class="form-control QTY text-primary" ></td>

										<td><input name="HARGA[]" onclick="select()" onkeyup="hitung()" value="{{$setord->HARGA}}" id="HARGA{{$no}}" type="text" style="text-align: right"  class="form-control HARGA text-primary" ></td>
       
										<td><input name="TOTAL[]" onclick="select()" onkeyup="hitung()" value="{{$setord->TOTAL}}" id="TOTAL{{$no}}" type="text" style="text-align: right"  class="form-control TOTAL text-primary" readonly></td>
       	   
                                        <td>
                                            <input name="BB_BP[]" id="BB_BP{{$no}}" type="text" value="{{$setord->BB_BP}}" class="form-control SATUAN" readonly required>
                                        </td>
				...
				success: function( response )
				{
					...
					for(i=0; i<resp.length; i++){
						...
						dTableBBarang.row.add([
							'<a href="javascript:void(0);" onclick="chooseBarang(\''+resp[i].KD_BRG+'\',\''+resp[i].NA_BRG+'\', \''+resp[i].SATUAN+'\' , \''+resp[i].BB_BP+'\' )">'+resp[i].KD_BRG+'</a>',
							resp[i].NA_BRG,
							resp[i].SATUAN,
							resp[i].BB_BP,
						]);
...
//////////////////////////////////////////////////////////////////
    ...
    function tambah() {
        ...
        td4.innerHTML = "<input name='SATUAN[]'   id=SATUAN" + idrow + " type='text' class='form-control  SATUAN' required readonly>";		
 		td5.innerHTML = "<input name='QTY[]'  onblur='hitung()' value='0' id=QTY" + idrow + " type='text' style='text-align: right' class='form-control QTY  text-primary' required >";
 		td6.innerHTML = "<input name='HARGA[]'  onblur='hitung()' value='0' id=HARGA" + idrow + " type='text' style='text-align: right' class='form-control HARGA  text-primary' required >";
 		td7.innerHTML = "<input name='TOTAL[]'  onblur='hitung()' value='0' id=TOTAL" + idrow + " type='text' style='text-align: right' class='form-control TOTAL  text-primary' required readonly>";
        td8.innerHTML = "<input name='BB_BP[]'   id=BB_BP" + idrow + " type='text' class='form-control  BB_BP' required readonly>";		
